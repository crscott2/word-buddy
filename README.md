# Spell Buddy (Spelling Hangman) — v1.10

Private prototype: a kid-friendly spelling Hangman PWA for ~5-year-olds learning school words on **iPad in landscape**.

## Features
- **Landscape-first layout (v1.10)**: hangman/beach stage beside word blanks + large letter keyboard (stacked fallback in portrait)
- Compact header in landscape; **lose-only** outcome popup (big bold word + Next) sized for short landscape height
...
- **Lose sequence**: living craft pirate (hanging) → silly felt/cardboard skeleton (still hanging briefly) → trap opens → **detaches and drops ALL THE WAY DOWN** through the trap + dust puff → **3s pause** → outcome popup showing **only the secret word (bold)** + Next word
- **Win (v1.10)**: Pip frees from the noose, drops to the plank, grabs the treasure, and runs off-screen with a silly run — **no outcome popup**; auto-advances to the next word after the run
