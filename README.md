# Spell Buddy (Spelling Hangman) — v1.22

Private prototype: a kid-friendly spelling Hangman PWA for ~5-year-olds learning school words on **iPad in landscape**.

## Features
- **iOS-safe audio (v1.22)**: HTML5 `Audio` (playsInline) primary path — looping beach WAV + pirate miss WAV + unmute blip (works when Web Audio is silenced by the iPhone ringer switch); Web Audio unlocked in parallel (resume + silent buffer); gesture unlock stays armed until audible; mute / letter / Parents / “Tap 🔊 for sound” unlock; unmute plays an audible blip in the same tap; mute preference still in localStorage
